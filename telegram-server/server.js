...
const ALLOWED_ORIGIN =
  ...
  'https://sushi-house-9clk.onrender.com';
...
function formatOrder(order) {
  ...
  for (
    const item of Array.isArray(order.items)
      ? order.items.slice(0, 50)
      : []
  ) {
    ...
    lines.push(
      `• ${name} × ${qty} — ${(price * qty).toLocaleString(
        'ru-RU'
      )} ₽`
    );
  }
...
app.post('/api/order', async (req, res) => {
  ...
  if (
    !clean(order.name, 100) ||
    !clean(order.phone, 40) ||
...
app.listen(PORT, () => {
  console.log(
    `Sushi House Telegram server listening on ${PORT}`
  );
});
